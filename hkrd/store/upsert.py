...
def upsert_odds_doubles(conn: sqlite3.Connection, rows: Sequence[Row]) -> int:
    ...
    stores its pair with horse_a < horse_b so one quinella cannot have two
    contradictory rows; doing the same here would be a bug, because a double IS
    ordered -- first-leg 3 with second-leg 7 is a different bet at a different
    price from 7 then 3, and sorting would leave whichever was written last
    standing for both.
    """
    prepared = [{
        "race_date": coerce.to_date(r.get("race_date")),
        "leg_no": coerce.to_int(r.get("leg_no"), field="leg_no"),
        "race_first": coerce.to_int(r.get("race_first"), field="race_first"),
        "race_second": coerce.to_int(r.get("race_second"), field="race_second"),
    ...
    cols = ["race_date", "leg_no", "race_first", "race_second",
            "horse_first", "horse_second", "captured_at", "odds"]
...
def upsert_pool_turnover(conn: sqlite3.Connection, rows: Sequence[Row]) -> int:
    ...
    exactly as it is for prices. The series IS the data: one figure is a size,
    two are a rate.

    A NULL amount is stored as NULL and not skipped. "This pool is not open"
    is a fact about the race, and a row saying so is what makes it different
    from a capture that never ran.
    """
    prepared = []
    for r in rows:
